...
def calc_G(H,X,G,O,K,t2,tau,envs,pg,rg):
    ...
    for j in range(envs):
        for taus in range(tau):
            Gt[j,ts] = G[j,ts]*np.sum(np.sum(O[:,j,slice(max(0,t2-taus),t2)]))* sum(sum(H.T.dot(Y)))\
             / ( np.sum(np.sum(O[:,j,slice(max(0,t2-taus),t2)]))*sum(sum(H.T.dot(X))) + pg*rg *np.power(abs(G[j,ts]),pg))
    return Gt
